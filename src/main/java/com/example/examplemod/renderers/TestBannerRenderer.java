...
import com.example.examplemod.BetterBanners;
...
public class TestBannerRenderer implements BlockEntityRenderer<TestBannerBlockEntity> {
   public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(new ResourceLocation(BetterBanners.MODID, "testbanner"), "main");
   ...
   public TestBannerRenderer(BlockEntityRendererProvider.Context p_173521_) {
   ...
   public void render(TestBannerBlockEntity p_112052_, float p_112053_, PoseStack p_112054_, MultiBufferSource p_112055_, int p_112056_, int p_112057_) {
      ...
      if (flag) {
         ...
         if (blockstate.getBlock() instanceof TestBannerBlock) {
            p_112054_.translate(0.5D, 0.5D, 0.5D);
            float f1 = (float)(-blockstate.getValue(TestBannerBlock.ROTATION) * 360) / 16.0F;
            p_112054_.mulPose(Vector3f.YP.rotationDegrees(f1));
            this.pole.visible = true;
         } else {
            p_112054_.translate(0.5D, (double)-0.16666667F, 0.5D);
            float f3 = -blockstate.getValue(TestWallBannerBlock.FACING).toYRot();
...
//         float[] afloat = pair.getSecond().getTextureDiffuseColors();
         ...
         p_112079_.render(p_112075_, material.buffer(p_112076_, RenderType::entityNoOutline), p_112077_, p_112078_, 1, 1, 1, 1.0F);
